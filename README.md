...
Self-hosted documentation browser for the workspace workspace. Reads markdown from all project `docs/` folders and renders them with rich formatting.

## Overview

A Hono backend + React frontend that auto-discovers all projects under `~/workspace/projects/`, renders their markdown files with syntax highlighting, diagrams, and a table of contents, and live-reloads the browser when files change.
...
```bash
cd ~/workspace/projects/docs-browser
npm install
...
**Part of:** `/example/workspace/` workspace
