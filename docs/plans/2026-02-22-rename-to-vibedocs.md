...
```bash
mv ~/workspace/projects/docs-browser ~/workspace/projects/vibedocs
```

Expected: `~/workspace/projects/vibedocs/` exists, old path gone.
...
- Modify: `~/workspace/CLAUDE.md` — lines 96-102 (Docs Browser section, paths, service commands)
- Modify: `~/workspace/README.md` — lines 17, 152, 170, 178-179 (table, section, paths, GitHub URL)
- Modify: `~/workspace/docs/port-registry.md` — line 16 (port entry)
...
```bash
mv ~/workspace/projects/vibedocs/systemd/docs-browser.service ~/workspace/projects/vibedocs/systemd/vibedocs.service
```
...
```bash
cd ~/workspace/projects/vibedocs
...
```bash
grep -r "docs-browser" ~/workspace/projects/vibedocs/ --include="*.md" --include="*.json" --include="*.sh" --include="*.service" || echo "Clean"
grep -r "docs-browser" ~/workspace/CLAUDE.md ~/workspace/README.md ~/workspace/docs/port-registry.md || echo "Clean"
```
