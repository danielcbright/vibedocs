...
describe('createApiClient', () => {
  ...
  describe('getProjects', () => {
    it('fetches /api/projects with the fileType query and returns json.data', async () => {
      fetchSpy.mockResolvedValueOnce(
        new Response(
          JSON.stringify({ data: [{ name: 'demo', hasDocsFolder: true, tree: [] }] }),
      ...
      expect(result).toEqual([{ name: 'demo', hasDocsFolder: true, tree: [] }])
    })
  ...
  describe('renderDoc', () => {
    it('fetches /api/render/:project/:path and returns { html, toc }', async () => {
      ...
      const result = await client.renderDoc('demo', 'docs/install.md')

      expect(fetchSpy).toHaveBeenCalledWith(
        '/api/render/demo/docs/install.md',
        expect.anything(),
      )
  ...
  describe('getRawDoc', () => {
    it('fetches /api/raw/:project/:path and returns the body as text', async () => {
      ...
      const result = await client.getRawDoc('demo', 'README.md')

      expect(fetchSpy).toHaveBeenCalledWith('/api/raw/demo/README.md', expect.anything())
