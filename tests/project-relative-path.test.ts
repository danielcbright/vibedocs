...
describe('toProjectRelativePath', () => {
  const projectsDir = '/home/dbright/claudebot/projects'
  ...
  it('handles a deeply nested file', () => {
    const abs = path.join(projectsDir, 'argus', 'docs', 'specs', '2026', 'plan.md')
    expect(toProjectRelativePath(abs, projectsDir)).toBe(
      'argus/docs/specs/2026/plan.md',
    )
  })
  ...
  it('returns null when the path is outside the projects dir', () => {
    ...
    expect(
      toProjectRelativePath('/home/dbright/elsewhere/file.md', projectsDir),
    ).toBeNull()
  })
  ...
  it('returns null for a path that resolves outside via traversal', () => {
    ...
    expect(
      toProjectRelativePath('/home/dbright/claudebot/other/foo.md', projectsDir),
    ).toBeNull()
  })
