...
- **Never trust `npm publish --dry-run` here — it fails for a reason that has nothing to do with publishing.** `npm_config_dry_run=true` propagates from the outer command into every nested npm call, so the `cd frontend && npm install` inside `npm run build` installs *nothing*. Vite then dies on `Cannot find package '@vitejs/plugin-react'` / `Could not resolve '@tailwindcss/vite'`, which reads like a broken `frontend/vite.config.ts` and sends you debugging a config that is fine. The tell is `frontend/node_modules` being absent afterwards. **The valid pre-flight is a real `npm pack`** (that is exactly what the `publish-rehearsal` CI job runs), then inspect the tarball with `tar -tzf`. To publish the audited bytes rather than a fresh unverified build, pack first and publish the tarball path: `npm publish ./vibedocs-<v>.tgz`.
